 ...
 *   4. catalog.default_provider
 */
export function resolveProvider(options = {}, catalog) {
 ...
 * (e.g. "z-ai/glm-5.2" for nous, "glm-5.2" for ollama).
 */
...
/**
 * Pick a model for a given role. Returns { model, provider, modelId, role, reasoningEffort }.
 * @param {string} role - one of: default, worker, ultrabrain, planner, verifier, quick
 * @param {object} options - { provider?, model? } overrides
 */
export async function pickModel(role = "default", options = {}) {
  const catalog = await loadCatalog();
  const provider = resolveProvider(options, catalog);

  // explicit --model wins, but still resolve to provider-specific ID
  const roleEntry = catalog.roles?.[role] || catalog.roles?.default || {};
  const canonical = options.model || roleEntry.model || catalog.current?.model || "glm-5.2";
